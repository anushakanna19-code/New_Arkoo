...
export function MeetingDetail({ meeting, onBack, onDelete, profile, employees = [] }: { meeting: any, onBack: () => void, onDelete: (id: string) => void, profile?: any, employees?: any[] }) {
  ...
  const effectiveDriveUrl = useMemo(() => {
    return currentMeeting?.driveFileUrl || currentMeeting?.backupDriveFileUrl || currentMeeting?.driveUrl || currentMeeting?.gdriveUrl || null;
  }, [currentMeeting]);
  ...
  const handleLocalRetryUpload = async () => {
    ...
    try {
      let res = await fetch(getApiUrl(`/api/gdrive/retry-upload/${currentMeeting.id}`), {
        method: 'POST'
      });
      if (!res.ok) {
        res = await fetch(getApiUrl(`/api/meetings/${currentMeeting.id}/retry-drive-upload`), {
          method: 'POST'
        });
      }
      if (!res.ok) {
      ...
      setCurrentMeeting((prev: any) => ({
        ...
        backupDriveFileUrl: data.driveFileUrl,
      }));
    } catch (err: any) {
    ...
    <div className="space-y-5 max-w-6xl mx-auto pb-20 font-sans">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            className="h-9 rounded-xl px-3.5 border-blue-200 text-blue-700 bg-blue-50/70 font-semibold text-xs shadow-xs hover:bg-blue-100/80 cursor-pointer flex items-center gap-1.5"
            title="Re-run AI analysis on transcript to regenerate MOM and Action Items"
            id="header-regenerate-mom-btn"
          >
            <Sparkles className={`w-3.5 h-3.5 text-blue-600 ${isRegeneratingMom ? 'animate-spin' : ''}`} />
            {isRegeneratingMom ? 'Regenerating...' : 'Regenerate MOM'}
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              ...
              <button
                ...
                onClick={() => handleRegenerateMom()}
                disabled={isRegeneratingMom}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 text-xs font-bold transition-all disabled:opacity-50 cursor-pointer border border-blue-100"
                title="Regenerate structured MOM from meeting transcript"
                id="mom-card-regenerate-btn"
              >
                <RefreshCw className={`w-3 h-3 ${isRegeneratingMom ? 'animate-spin' : ''}`} />
                {isRegeneratingMom ? 'Regenerating...' : 'Regenerate MOM'}
              </button>
            </div>
          ...
          {(currentMeeting.audioUrl || effectiveDriveUrl) && (
            <MeetingAudioPlayer 
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm space-y-2.5">
            <Button 
              onClick={() => handleRegenerateMom()}
              disabled={isRegeneratingMom}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold uppercase text-[10px] tracking-widest shadow-md h-10 rounded-xl cursor-pointer flex items-center justify-center gap-2"
              id="sidebar-regenerate-mom-btn"
            >
              <Sparkles className={`w-3.5 h-3.5 ${isRegeneratingMom ? 'animate-spin' : ''}`} />
              {isRegeneratingMom ? 'Regenerating MOM...' : 'Regenerate MOM'}
            </Button>

            <Button 
              onClick={handleDownloadReport}
              disabled={isGeneratingReport}
              className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold uppercase text-[10px] tracking-widest shadow-md h-10 rounded-xl cursor-pointer"
